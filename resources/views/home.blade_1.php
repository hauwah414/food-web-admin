...
@section('page-script')
    ...
    <script
        ...
        type="text/javascript"></script>
    <script src="{{ env('STORAGE_URL_VIEW') }}{{ 'assets/pages/scripts/table-datatables-scroller.min.js' }}"
        type="text/javascript"></script>
    <script>
        $('.sample').DataTable({
            "searching": false,
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3" style="text-align:right;margin-top:20px">
                                @if (strpos($thn, 'last') !== false)
                                    <select id="bulane" class="form-control" name="thn"
                                        onChange="gantiHome(this.value);">
                                        <option value="last7days" @if ($thn == 'last7days') selected @endif>Last 1
                                            Week</option>
                                        <option value="last30days" @if ($thn == 'last30days') selected @endif>Last 30
                                            Days</option>
                                        <option value="last3months" @if ($thn == 'last3months') selected @endif>Last 3
                                            Months</option>
                                    </select>
                                @else
                            ...
                            @if ($thn == 'alltime' || strpos($thn, 'last') !== false)
                                <div class="col-lg-3" style="text-align:right;margin-top:20px">
                                    <a href="{{ URL::to('home/' . date('Y') . '/' . date('m')) }}"
                                        class="btn grey btn-block">This Month</a>
                                </div>
                            @else
                                <div class="col-lg-2" style="text-align:right;margin-top:20px">
                                    <select id="bulane" class="form-control" name="month" onChange="gantiHome();">
                                        <option value="1" @if ($bln == '1') selected @endif>January
                                        </option>
                                        <option value="2" @if ($bln == '2') selected @endif>February
                                        </option>
                                        <option value="3" @if ($bln == '3') selected @endif>March
                                        </option>
                                        <option value="4" @if ($bln == '4') selected @endif>April
                                        </option>
                                        <option value="5" @if ($bln == '5') selected @endif>May
                                        </option>
                                        <option value="6" @if ($bln == '6') selected @endif>June
                                        </option>
                                        <option value="7" @if ($bln == '7') selected @endif>July
                                        </option>
                                        <option value="8" @if ($bln == '8') selected @endif>August
                                        </option>
                                        <option value="9" @if ($bln == '9') selected @endif>September
                                        </option>
                                        <option value="10" @if ($bln == '10') selected @endif>October
                                        </option>
                                        <option value="11" @if ($bln == '11') selected @endif>November
                                        </option>
                                        <option value="12" @if ($bln == '12') selected @endif>December
                                        </option>
                                    </select>
                                </div>
                                <div class="col-lg-2" style="text-align:right;margin-top:20px">
                                        ...
                                        @foreach ($item['dashboard_card'] as $card)
                                                    ...
                                                    <div class="portlet light bordered">
                                                        <div class="portlet-title">
                                                            <div class="caption">
                                                                ...
                                                                <span
                                                                    class="caption-subject font-dark bold uppercase">{{ $card['card_name'] }}</span>
                                                            </div>
                                                        </div>
                                                        <div class="portlet-body">
                                                            @if (strpos($card['card_name'], 'Table') !== false)
                                                                <table
                                                                    class="table table-striped table-bordered table-hover order-column sample"
                                                                        ...
                                                                        <thead>
                                                                            <tr>
                                                                                ...
                                                                                @foreach (array_keys($card['value'][0]) as $header)
                                                                                    @if ($header != 'id')
                                                                                        <th> {{ ucfirst(str_replace('_', ' ', $header)) }}
                                                                                        </th>
                                                                                    @endif
                                                                                @endforeach
                                                                                <th>action</th>
                                                                                ...
                                                                                <tr>
                                                                                    ...
                                                                                    <td>
                                                                                        <a class="btn btn-block yellow btn-xs"
                                                                                            @if (strpos($card['card_name'], 'User') !== false || strpos($card['card_name'], 'Customer') !== false) href="{{ url('report/customer/detail/' . $value[$head[1]] . '/transactions') }}">
																					...
																					@elseif(strpos($card['card_name'], 'Outlet') !== false)
																						href="{{ url('report/outlet/detail/' . $value[$head[1]] . '/' . $card['url']) }}"> @endif
                                                            ...
                                                            @else
                                                                <div id="chart-{{ $card['id_dashboard_card'] }}"
                                                                    class="chartdiv"></div>
                                                                @if (!empty($card['value']))
                                            ...
                                            @else
                                                <div class="col-md-4" style="margin-top:20px">
                                                    <div class="dashboard-stat grey blue_">
                                                        <div class="visual">
                                                        ...
                                                        <div class="details">
                                                            <div class="number">
                                                                <span data-counter="counterup"
                                                                    data-value="{{ $card['value'] }}">{{ number_format($card['value'], 0, '.', ',') }}</span>
                                                            </div>
                                                            <div class="desc">
                                                                {{ $card['card_name'] }}
                                                            </div>
                                                        ...
                                                        @if (strpos($card['card_name'], 'Customer') !== false ||
                                                                strpos($card['card_name'], 'Admin') !== false ||
                                                                strpos($card['card_name'], 'User') !== false)
                                                            <a class="more"
                                                                href="{{ url('report/customer/summary/?' . $card['url']) }}">
                                                                @if (strpos($card['card_name'], 'New') !== false)
                                                                    Register Within {{ $dashboard['daterange'] }}
                                                                @else
                                                                    @if (strpos($dashboard['daterange'], 'days') !== false ||
                                                                            strpos($dashboard['daterange'], 'months') !== false ||
                                                                            strpos($dashboard['daterange'], 'time') !== false)
                                                                        {{ str_replace('Total', '', $card['card_name']) }}
                                                                        (All Time)
                                                                    @else
                                                                        {{ str_replace('Total', '', $card['card_name']) }}
                                                                        (Until {{ $dashboard['daterange'] }})
                                                                    @endif
                                                                @endif
