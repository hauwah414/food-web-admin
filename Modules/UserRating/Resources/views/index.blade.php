            ...
            <div class="portlet-body form">
                <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="rating_table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><a href="{{url('user-rating/detail/'.$rating['id_user_rating']??null)}}" class="btn blue btn-sm">Detail</a></td>
                            <td>{{date('d M Y',strtotime($rating['created_at']??null))}}</td>
                            <td><a href="{{url('transaction/detail'.'/').$rating['transaction']['id_transaction']??null}}">{{$rating['transaction']['transaction_receipt_number']??null}}</a></td>
                            <td><a href="{{url('user/detail'.'/'.$rating['user']['phone']??null)}}">{{$rating['user']['name']??null}}</a></td>
                            <td>Rp {{number_format($rating['transaction']['transaction_grandtotal']??0,0,',','.')}}</td>
                            <td>
                               {{$rating['product']['product_name']??null}}
                            <td>{{ $rating['rating_value']??null }}</td>
                            <td>{{ $rating['option_value']??null }}</td>
                        </tr>
                        @endforeach
                        @else
                        <tr>
