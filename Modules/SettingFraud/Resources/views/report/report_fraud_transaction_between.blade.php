...
@section('content')
    ...
    @if (Session::has('filter-fraud-log-trx-minute'))
        ...
        <div class="alert alert-block alert-success fade in">
            ...
            <br>
            <p>
                <a href="{{ url('fraud-detection/filter/reset') }}/filter-fraud-log-trx-minute"
    ...
    @if (!empty($result))
        <div style="text-align: right">
            <a class="btn blue" href="{{ url('fraud-detection/report/transaction-between') }}?export-excel=1"><i
                    ...
                    <tbody>
                        @if (!empty($result))
                            @foreach ($result as $value)
                                <tr>
                                    <td>
                                        <a href="{{ url('fraud-detection/report/detail/transaction-between/' . $value['id_user'] . '/' . date('Y-m-d', strtotime($value['created_at']))) }}"
                                            target="_blank" class="btn btn-block blue btn-xs"><i class="fa fa-edit"></i>
                                            Detail</a>
                                    </td>
                                    <td>{{ $value['name'] }}</td>
                                    <td>{{ $value['phone'] }}</td>
                                    ...
                                    <td>
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-12"><input class="form-control" disabled
                                                    value="(bellow) {{ $value['fraud_setting_parameter_detail'] }}"></div>
                                        </div>
                                        <div class="form-group row">
                                        ...
                                        @if ($value['fraud_setting_auto_suspend_status'] == 1)
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-12"><input class="form-control" disabled
                                                        value="(maximum) {{ $value['fraud_setting_auto_suspend_value'] }}">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label class="col-md-12">Auto Suspend Time Period</label>
                                                <div class="col-md-12"><input class="form-control" disabled
                                                        value="{{ $value['fraud_setting_auto_suspend_time_period'] }} (day)">
                                                </div>
                                            </div>
                                        @endif
                                        <div class="form-group row">
