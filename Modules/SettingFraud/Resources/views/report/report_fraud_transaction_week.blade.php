        ...
        function updateStatusLog(id_fraud_detection_log_transaction_week, status) {
            ...
            if (confirm('Are you sure you want to delete this log?')) {
                $.ajax({
                    ...
                    data: "_token=" + token + "&id_fraud_detection_log_transaction_week=" +
                        id_fraud_detection_log_transaction_week + "&status=" + status,
...
@section('content')
                        ...
                        <p>{{ ucwords(str_replace('_', ' ', $row['subject'])) }}
                            @if ($row['subject'] != 'all_user')
                                @if ($row['subject'] == 'outlet')
                                    ...
                                    @endforeach
                                    {{ $name }}
                                @elseif($row['parameter'] != '')
                                    {{ str_replace('-', ' - ', $row['operator']) }}
                                    {{ str_replace('-', ' - ', $row['parameter']) }}
                                @else
                                    : {{ str_replace('-', ' - ', $row['operator']) }}
                                @endif
                            @endif
    ...
    @if (!empty($result))
        <div style="text-align: right">
            <a class="btn blue" href="{{ url('fraud-detection/report/transaction-week') }}?export-excel=1"><i
            ...
            <tbody>
                @if (!empty($result))
                    @foreach ($result as $value)
                        <tr>
                            <td><a href="{{ url('fraud-detection/report/detail/transaction-week') }}/{{ $value['id_fraud_detection_log_transaction_week'] }}"
                                    target="_blank" class="btn btn-block blue btn-xs"><i class="fa fa-edit"></i> Detail</a>
                            ...
                            <td>
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6"><input class="form-control" disabled
                                            value="(maximum) {{ $value['fraud_setting_parameter_detail'] }}"></div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-6">Auto Suspend</label>
                                    <div class="col-md-6"><input class="form-control" disabled
                                            value="@if ($value['fraud_setting_auto_suspend_status'] == 1) Active @else Inactive @endif"></div>
                                </div>
                                @if ($value['fraud_setting_auto_suspend_status'] == 1)
                                    <div class="form-group row">
                                        <label class="col-md-6">Auto Suspend Value</label>
                                        <div class="col-md-6"><input class="form-control" disabled
                                                value="(maximum) {{ $value['fraud_setting_auto_suspend_value'] }}"></div>
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-md-6">Auto Suspend Time Period</label>
                                        <div class="col-md-6"><input class="form-control" disabled
                                                value="{{ $value['fraud_setting_auto_suspend_time_period'] }} (day)"></div>
                                    </div>
                                @endif
                                <div class="form-group row">
                                    <label class="col-md-6">Forward Admin</label>
                                    <div class="col-md-6"><input class="form-control" disabled
                                            value="@if ($value['fraud_setting_forward_admin_status'] == 1) Active @else Inactive @endif"></div>
                                </div>
                            </td>
                        </tr>
                    @endforeach
