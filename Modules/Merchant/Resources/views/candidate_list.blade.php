...
@section('page-plugin')
    ...
    <script src="{{ env('STORAGE_URL_VIEW') }}{{ 'assets/global/plugins/jquery-repeater/jquery.repeater.js' }}"
        type="text/javascript"></script>
@endsection
...
@section('page-script')
    ...
    <script src="{{ env('STORAGE_URL_VIEW') }}{{ 'assets/global/plugins/bootstrap-toastr/toastr.min.js' }}"
        type="text/javascript"></script>
    <script src="{{ env('STORAGE_URL_VIEW') }}{{ 'assets/global/scripts/datatable.js' }}" type="text/javascript">
    </script>
    ...
    <script src="{{ env('STORAGE_URL_VIEW') }}{{ 'assets/global/plugins/bootstrap-sweetalert/sweetalert.min.js' }}"
        type="text/javascript"></script>
    <script src="{{ env('STORAGE_URL_VIEW') }}{{ 'assets/pages/scripts/form-repeater.js' }}" type="text/javascript">
    </script>
    <script>
                        ...
                        $(this).click(function() {
                            swal({
                                ...
                                function() {
                                    $.ajax({
                                        type: "POST",
                                        url: "{{ url('merchant/candidate/delete') }}" +
                                            '/' + id,
                                        ...
                                        success: function(result) {
                                            if (result.status == "success") {
                                                ...
                                                swal("Error!", result.messages[0],
                                                    "error")
                                            } else {
                                                swal("Error!",
                                                    "Something went wrong. Failed to delete candidate.",
                                                    "error")
                                            }
                                        }
                                    });
                                    ...
                                    <td>
                                        @if (MyHelper::hasAccess([324, 326], $grantedFeature))
                                            <a class="btn btn-sm btn-info"
                                                href="{{ url('merchant/candidate/detail', $val['id_merchant']) }}"><i
                                                    class="fa fa-search"></i></a>
                                        @endif
                                        @if (MyHelper::hasAccess([327], $grantedFeature))
                                            <a class="btn btn-sm red sweetalert-delete btn-primary"
                                                data-id="{{ $val['id_merchant'] }}"
                                                data-name="{{ $val['outlet_name'] }}"><i class="fa fa-trash-o"></i></a>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($val['merchant_status'] == 'Pending')
                                            <span class="sbold badge badge-pill"
                                                style="font-size: 14px!important;height: 25px!important;background-color: #ffe066;padding: 5px 12px;color: #fff;">Pending</span>
                                        @elseif($val['merchant_status'] == 'Rejected')
                                            <span class="sbold badge badge-pill"
                                                style="font-size: 14px!important;height: 25px!important;background-color: #E7505A;padding: 5px 12px;color: #fff;">Rejected</span>
                                        @else
                                            <span class="sbold badge badge-pill"
                                                style="font-size: 14px!important;height: 25px!important;background-color: #faf21e;padding: 5px 12px;color: #fff;">{{ $val['merchant_status'] }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($val['merchant_completed_step'] == 1)
                                            <span class="sbold badge badge-pill"
                                                style="font-size: 14px!important;height: 25px!important;background-color: #1BBC9B;padding: 5px 12px;color: #fff;">Completed</span>
                                        @else
                                            <span class="sbold badge badge-pill"
                                                style="font-size: 14px!important;height: 25px!important;background-color: #E87E04;padding: 5px 12px;color: #fff;">Not
                                                Complete</span>
                                        @endif
                                    </td>
