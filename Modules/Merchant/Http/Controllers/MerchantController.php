...
class MerchantController extends Controller
{
    ...
    public function create()
    {
        ...
        $data['bank'] = MyHelper::get('disburse/bank')['result'] ?? [];
    ...
    public function update(Request $request, $id)
    {
        ...
        if (isset($update['status']) && $update['status'] == "success") {
            ...
            return back()->withErrors($update['messages'] ?? ['Failed get data']);
        }
    }
    ...
    public function withdrawalListDetail($id)
    {
       ...
       $detail = MyHelper::get('merchant/withdrawal/detail/'.$id);

       if (isset($detail['status']) && $detail['status'] == "success") {
            $data['detail'] = $detail['result'];
            return view('merchant::withdrawal_detail', $data);
        } else {
            return redirect('merchant/candidate')->withErrors($save['messages'] ?? ['Failed get data']);
        }
    }
     public function withdrawalListExport($id){
        $dataOutlet = MyHelper::get('merchant/withdrawal/export/'.$id)['result']??[];
        return Excel::download(new ArrayExport($dataOutlet,'Tagihan Transaction'),date('YmdHis').'_Tagihan_Transaction.xlsx');
    }
