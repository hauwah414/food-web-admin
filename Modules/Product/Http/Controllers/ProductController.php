...
class ProductController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (empty($post)) {
            ...
            if (!empty($post['product_type'])) {
                $post['product_type'] = 'box';
                $post['base_price'] = 0;
            }else{
                $post['product_type']='product';
            }
            
            $post['admin'] = 1;
            $post['value_preorder'] = (int)$post['value_preorder']??1;
            $post['min_transaction'] = (int)$post['min_transaction']??1;
    ...
    public function listProductInactive(Request $request)
    {
        $post = $request->all();
        $data = [
            'title'          => 'Product',
            'sub_title'      => 'List Product Inactive',
            'menu_active'    => 'product',
            'submenu_active' => 'product-list-inactive'
        ];
        ...
        $data['categories'] = $resCat;
        return view('product::product.inactive', $data);
    }
    ...
    public function detail(Request $request, $code)
    {
        ...
        if (isset($detail['status']) && $detail['status'] == "success") {
            ...
            if($data['detail']['is_approved']==0){
                $data['submenu_active']= 'product-list-inactive';
            }
        } else {
            $e = ['e' => 'Data product not found.'];
            return redirect('product')->witherrors($e);
        }
        if($data['detail']['product_type']=='box'){
           $data['detail']['product'] = MyHelper::post('product/merchant', ['id_outlet'=>$data['detail']['id_outlet']])['result']??[];
        }
        $post = $request->except('_token');
        ...
        if (empty($post)) {
            ...
            $data['user'] = MyHelper::post('product/customer', $request->all())['result'] ?? [];
            $data['spesial'] = MyHelper::post('product/spesial_price', ['id_product'=>$data['detail']['id_product']])['result'] ?? [];
            $data['product_photo'] = MyHelper::post('product/multiple-photo', ['id_product'=>$data['detail']['id_product']])['result'] ?? [];
    ...
    {
        $post = $request->except('_token');
        $data = MyHelper::post('product/merchant', $post);
        return $data;
    }
    public function createPrice(Request $request)
    {
        ...
        $save = MyHelper::post('product/spesial_price/create', $post);
        if (isset($save['status']) && $save['status'] == "success") {
            return back()->withSuccess(['Create data success']);
        } else {
            return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
        }
    }
    ...
    {
        $save = MyHelper::post('product/spesial_price/delete', ['id_product_price_user'=>$id]);
        if (isset($save['status']) && $save['status'] == "success") {
            return back()->withSuccess(['Create data success']);
        } else {
            return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
    ...
    public function createPhoto(Request $request)
    {
        ...
        if (isset($post['product_image'])) {
            $post['product_image']      = MyHelper::encodeImage($post['product_image'], $request->file('product_image')->extension());
        }
        $save = MyHelper::post('product/multiple-photo/create', $post);
        if (isset($save['status']) && $save['status'] == "success") {
            return back()->withSuccess(['Create data success']);
        } else {
            return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
    ...
    public function deletePhotoMultiple($id)
    {
       $save = MyHelper::post('product/multiple-photo/delete', ['id_product_multiple_photo'=>$id]);
        if (isset($save['status']) && $save['status'] == "success") {
            return back()->withSuccess(['Create data success']);
        } else {
            return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
    ...
    {
        $post['product_code'] = $id;
        $save = MyHelper::post('product/be/approved', $post);
        if (isset($save['status']) && $save['status'] == "success") {
            return back()->withSuccess(['Approved product success']);
        } else {
            return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
        }
    }
