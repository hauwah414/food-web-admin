...
class UsersController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $getDepartment= MyHelper::get('department/list');

            if ($getDepartment['status'] == 'success') {
                $data['department'] = $getDepartment['result'];
            } else {
                $data['department'] = [];
            }
    ...
    public function department(Request $request)
    {
        $post = $request->except('_token');
          $data = [ 'title'             => 'Deparment',
            'subtitle'        => 'Deparment',
            'menu_active'       => 'deparment',
            'submenu_active'    => 'deparment-list'
        ];
        $post['page'] = $post['page'] ?? 1;

        $getUser = MyHelper::post('users/department', $post);
       
        $data['data']          = $getUser['result']??[];

        return view('users::department', $data);
    }
}
