...
class OutletController extends Controller
{
    ...
    */
    public function detail(Request $request, $code, $type = null)
    {
        ...
        if (empty($post['outlet_name'])) {
            ...
            $data['delivery'] = MyHelper::post('outlet/delivery', ['id_outlet'=>$data['id_outlet']])['result'] ?? [];
            $data['bank'] = MyHelper::post('merchant/be/bank-account/list', ['id_outlet' => $data['id_outlet']])['result'] ?? [];
            $data['ongkos_kirim'] = MyHelper::post('setting', ['key' => 'ongkos_kirim'])['result']['value'] ?? 0;
    ...
    */
    ...
    public function createOngkir(Request $request)
    {
        $post = $request->except('_token');

        if (empty($post)) {
            $data = [
                'title'          => 'Outlet Delivery',
                'sub_title'      => 'New Outlet Delivery',
                'menu_active'    => 'outlet-delivery',
                'submenu_active' => 'outlet-delivery',
            ];
            $data['outlets'] = MyHelper::get('outlet/be/list')['result'] ?? [];
            $data['ongkos'] = MyHelper::post('setting', ['key' => 'default_ongkos_kirim'])['result']['value'] ?? 0;
            $data['ongkos_flat'] = MyHelper::post('setting', ['key' => 'default_ongkos_kirim_flat'])['result']['value'] ?? 0;
            return view('outlet::createOngkir', $data);
        } else {
            $post['total_price'] = str_replace('.', '', $post['total_price']);
            $post['price_delivery'] = str_replace('.', '', $post['price_delivery']);
            $save = MyHelper::post('outlet/delivery/create', $post);
            if (isset($save['status']) && $save['status'] == "success") {
    ...
    {
       $save = MyHelper::post('outlet/delivery/delete', ['id_outlet_delivery'=>$id]);
            if (isset($save['status']) && $save['status'] == "success") {
                return back()->withSuccess(['Create data success']);
            } else {
                return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
    ...
    public function dataLegalitas(Request $request)
    {
        ...
        if (!empty($post['npwp_attachment'])) {
                $post['npwp_attachment_ext'] = pathinfo($post['npwp_attachment']->getClientOriginalName(), PATHINFO_EXTENSION);
                $post['npwp_attachment'] = MyHelper::encodeImage($post['npwp_attachment']);
        }
        if (!empty($post['nib_attachment'])) {
                $post['nib_attachment_ext'] = pathinfo($post['nib_attachment']->getClientOriginalName(), PATHINFO_EXTENSION);
                $post['nib_attachment'] = MyHelper::encodeImage($post['nib_attachment']);
        }
        $save = MyHelper::post('outlet/data-legalitas', $post);
        if (isset($save['status']) && $save['status'] == "success") {
            return back()->withSuccess(['Update data legalitas success']);
        } else {
            return back()->withErrors(['Something when wrong. Please try again.'])->withInput();
        }
    }
