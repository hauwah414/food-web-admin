                    ...
                    @if(!empty($data))
                        @foreach($data as $res)
                            <tr>
                                ...
                                <td>{{ $res['name_department'] }}</td>
                               <td>{{ date('d F Y H:i', strtotime($res['transaction_group_date'])) }}</td>
                                ...
                                <td>{{number_format($res['transaction_grandtotal'],0,",",".")}}</td>
                                 <td><span class="badge badge-sm {{$codeColor[$res['transaction_payment_status_code']]??'badge-default'}}" @if($res['transaction_payment_status_code'] == 4) style="background-color: #28a745;" @endif>{{ $res['transaction_payment_status'] }}</span></td>
                                <td>
                                    @if(MyHelper::hasAccess([39], $grantedFeature))
                                        <a class="btn btn-block yellow btn-xs" href="{{ url('transaction/group/detail', $res['transaction_receipt_number']) }}"><i class="icon-pencil"></i> Detail </a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
