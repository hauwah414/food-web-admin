...
class TransactionController extends Controller
{
    ...
    public function autoResponse(Request $request, $subject)
    {
        ...
        switch ($subject) {
            ...
            case 'merchant-transaction-new':
                ...
                break;
            case 'Reminder Pembayaran Tagihan':
                $data['click_inbox'] = [
                    ['value' => "Reminder Pembayaran Tagihan",'title' => 'Reminder Pembayaran Tagihan'],
                    ['value' => "reminder_pembayaran_tagihan",'title' => 'Reminder Pembayaran Tagihan']
                ];
                $data['click_notification'] = [
                    ['value' => "Reminder Pembayaran Tagihan",'title' => 'Reminder Pembayaran Tagihan'],
                    ['value' => "reminder_pembayaran_tagihan",'title' => 'Reminder Pembayaran Tagihan']
            ...
            case 'Pembayaran Tagihan':
                $data['click_inbox'] = [
                    ['value' => "Pembayaran Tagihan",'title' => 'Pembayaran Tagihan'],
                    ['value' => "pembayaran_tagihan",'title' => 'Pembayaran Tagihan']
                ];
                $data['click_notification'] = [
                    ['value' => "Reminder Pembayaran Tagihan",'title' => 'Pembayaran Tagihan'],
                    ['value' => "pembayaran_tagihan",'title' => 'Pembayaran Tagihan']
                ];
                break;
            default:
    ...
    public function transactionList(Request $request)
    {
        ...
        $data = [
            'title'          => 'Transaction',
            'menu_active'    => 'transaction',
            'sub_title'      => 'List Transaction Pending',
            'submenu_active' => 'transaction-pending',
        ...
        $list = MyHelper::post('transaction/be/list-all', $post);
    ...
    public function transactionListProcess(Request $request)
    {
        $post = $request->except('_token');
        $data = [
            'title'          => 'Transaction',
            'menu_active'    => 'transaction',
            'sub_title'      => 'List Transaction Process',
            'submenu_active' => 'transaction-process',
            'title_date_start' => 'Start',
            'title_date_end' => 'End',
        ];

        if (Session::has('filter-transaction-list-process') && !empty($post) && !isset($post['filter'])) {
            $page = 1;
            if (isset($post['page'])) {
                $page = $post['page'];
            }
            $post = Session::get('filter-transaction-list-process');
            $post['page'] = $page;
        } else {
            Session::forget('filter-transaction-list-process');
        }
        ...
        if ($post) {
            Session::put('filter-transaction-list-process', $post);
        }
        return view('transaction::transactionList', $data);
    }
    public function transactionListDelivery(Request $request)
    {
        $post = $request->except('_token');
        $data = [
            'title'          => 'Transaction',
            'menu_active'    => 'transaction',
            'sub_title'      => 'List Transaction Delivery',
            'submenu_active' => 'transaction-delivery',
            'title_date_start' => 'Start',
            'title_date_end' => 'End',
        ];

        if (Session::has('filter-transaction-list-delivery') && !empty($post) && !isset($post['filter'])) {
            $page = 1;
            if (isset($post['page'])) {
                $page = $post['page'];
            }
            $post = Session::get('filter-transaction-list-delivery');
            $post['page'] = $page;
        } else {
            Session::forget('filter-transaction-list-delivery');
        }
        ...
        if ($post) {
            Session::put('filter-transaction-list-delivery', $post);
        }
        return view('transaction::transactionList', $data);
    }
    public function transactionListCompleted(Request $request)
    {
        ...
        $data = [
            'title'          => 'Transaction',
            'menu_active'    => 'transaction',
            'sub_title'      => 'List Transaction Completed',
            'submenu_active' => 'transaction-completed',
            'title_date_start' => 'Start',
            'title_date_end' => 'End',
        ];

        if (Session::has('filter-transaction-list-completed') && !empty($post) && !isset($post['filter'])) {
            $page = 1;
            if (isset($post['page'])) {
                $page = $post['page'];
            }
            $post = Session::get('filter-transaction-list-completed');
            $post['page'] = $page;
        } else {
            Session::forget('filter-transaction-list-completed');
        }
        ...
        if ($post) {
            Session::put('filter-transaction-list-completed', $post);
        }
        return view('transaction::transactionListCompleted', $data);
    }
    public function transactionListCancel(Request $request)
    {
        $post = $request->except('_token');
        $data = [
            'title'          => 'Transaction',
            'menu_active'    => 'transaction',
            'sub_title'      => 'List Transaction Cancel',
            'submenu_active' => 'transaction-cancel',
            'title_date_start' => 'Start',
            'title_date_end' => 'End',
        ];
    ...
    public function transactionDetail($id)
    {
        ...
        if (isset($check['status']) && $check['status'] == "success") {
            ...
            $data['item'] = MyHelper::post('transaction/be/product', ['id_outlet' => $check['result']['outlet']['id_outlet'],'id_transaction' => $id])['result']??[];
            if($data['detail']['transaction_status_code']==3){
    ...
    {
           $uploadImage = MyHelper::post('merchant/be/transaction/accept', ['id_transaction'=>$id,'is_admin'=>1]);
            if (isset($uploadImage['status']) && $uploadImage['status'] == "success") {
                return redirect()->back()->withSuccess(['Accept transaksi. Segera proses']);
            } else {
                return redirect()->back()->withErrors($uploadImage['messages'] ?? ['Something when wrong. Failed upload image.'])->withInput();
            }
    }
    public function reject(Request $request)
    {
        $post = $request->except('_token');
        $post['is_admin'] = 1;
        $uploadImage = MyHelper::post('merchant/be/transaction/reject', $post);
        if (isset($uploadImage['status']) && $uploadImage['status'] == "success") {
            return redirect()->back()->withSuccess(['Reject transaction success']);
        } else {
            return redirect()->back()->withErrors($uploadImage['messages'] ?? ['Something when wrong. Failed upload image.'])->withInput();
        }
    }
    public function requestDelivery(Request $request,$id)
    {
        ...
        if (empty($post['pengirim'])) {
            return redirect()->back()->withErrors(['Contact Kurir Empty'])->withInput();
        }
         if (isset($post['attachment'])) {
            $post['attachment']      = MyHelper::encodeImage($post['attachment'], $request->file('attachment')->extension());
        }
        $post['id_transaction'] = $id;
        $post['is_admin'] = 1;
        $uploadImage = MyHelper::post('merchant/be/transaction/delivery/request', $post);
        
        if (isset($uploadImage['status']) && $uploadImage['status'] == "success") {
            return redirect()->back()->withSuccess(['Confirm transaction success']);
        } else {
            return redirect()->back()->withErrors($uploadImage['messages'] ?? ['Contact Kurir Empty'])->withInput();
        }
    }
    public function confirm(Request $request,$id)
    {
        ...
        $post['id_transaction'] = $id;
        $post['is_admin'] = 1;
        $uploadImage = MyHelper::post('merchant/be/transaction/delivery/confirm',$post);
        if (isset($uploadImage['status']) && $uploadImage['status'] == "success") {
            return redirect()->back()->withSuccess(['Confirm transaction success']);
        } else {
            return redirect()->back()->withErrors($uploadImage['messages'] ?? ['Something when wrong. Failed upload image.'])->withInput();
