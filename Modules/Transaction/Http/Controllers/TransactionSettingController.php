...
class TransactionSettingController extends Controller
{
    ...
    public function settingAllFee(Request $request, $type = null)
    {
        ...
        if (empty($post)) {
            ...
            if ($type == 'service') {
                ...
                $update = MyHelper::post('transaction/setting/ongkir', $post);
            }
    ...
    public function settingAllReminder(Request $request, $type = null)
    {
        $post = $request->all();
        ...
        if (empty($post)) {
            $data = [
                'title'          => 'Order',
                'menu_active'    => 'order',
                'sub_title'      => 'Setting Reminder',
                'submenu_active' => 'setting-reminder'
            ];

            $data['reminder'] = MyHelper::post('setting', ['key' => 'reminder-tagihan-pembayaran'])['result']['value'] ?? 0;
            $data['expired'] = MyHelper::post('setting', ['key' => 'expired-date-tagihan-pembayaran'])['result']['value'] ?? 0;
            $data['received'] = MyHelper::post('setting', ['key' => 'date-order-received'])['result']['value'] ?? 0;
            return view('transaction::setting.all_reminder', $data);
        } else {
            if ($type == 'service') {
                $service = $post['service'] / 100;
                $service = ($service == 0 ? '0.0' : $service);
                ...
                $update = MyHelper::post('transaction/setting/reminder', $post);
            } elseif ($type == 'expired') {
                $update = MyHelper::post('transaction/setting/expired', $post);
            } elseif ($type == 'received') {
                $update = MyHelper::post('transaction/setting/received', $post);
            }

            if (($update['status'] ?? false) == 'success') {
                return redirect('transaction/setting/all-reminder')->withSuccess(['Success update']);
            } else {
                return back()->withErrors(['Update failed']);
            }
        }
