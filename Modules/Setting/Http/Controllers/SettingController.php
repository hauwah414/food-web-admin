...
//        return $data;
    ...
    public function invoiceSetting(Request $request)
    {
        ...
        'menu_active'    => 'setting-invoice',
        'submenu_active' => 'setting-invoice',
        ];

        $data['data'] = MyHelper::get('setting/invoice')['result']??null;

 
       return view('setting::invoice', $data);
    }
    ...
    public function defaultInvoice(Request $request)
    {
       $post = $request->except('_token');
        if (isset($post['logo_its'])) {
            $post['logo_its'] = MyHelper::encodeImage($post['logo_its']);
        }
        if (isset($post['ttd_finance'])) {
            $post['ttd_finance'] = MyHelper::encodeImage($post['ttd_finance']);
        }
       $result = MyHelper::post('setting/saveInvoice', $post);
        return parent::redirect($result, 'Setting Invoice has been updated.');
    }
...
//    }
    public function whatsApp(Request $request)
    {
        ...
        $data = [
        'title'          => 'Setting Link WhatsApp',
        'menu_active'    => 'crm-setting-whatsapp',
        'submenu_active' => 'whatsapp',
        ];

        if (empty($post)) {
            $data['link'] = MyHelper::get('setting/whatsapp')['result']['value']??null;
            return view('setting::whatsApp.whatsapp', $data);
        } else {
            $save = MyHelper::post('setting/whatsapp', $post);
            if (isset($save['status']) && $save['status'] == "success") {
                return redirect('setting/cs')->withSuccess(['Link whatsApp has been updated.']);
            } else {
                if (isset($save['errors'])) {
                    return back()->withErrors($save['errors'])->withInput();
                }
...
/* Web Banner */
    public function createWebBanner(Request $request)
    {
        $validatedData = $request->validate([
        'banner_image' => 'required|dimensions:width=1400,height=375,ratio=2/1',
        ]);
    ...
    public function updateWebBanner(Request $request)
    {
        $validatedData = $request->validate([
        'id_banner'    => 'required',
        'banner_image' => 'sometimes|dimensions:width=750,height=375',
    ...
    public function reorderWebBanner(Request $request)
    {
        $post = $request->except("_token");
        // dd($post['id_banners']);
        $result = MyHelper::post('setting/banner/reorder', $post);
    ...
    // delete banner
    public function deleteWebBanner($id_banner)
    {
        $post['id_banner'] = $id_banner;
